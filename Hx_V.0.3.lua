...
local function applyPreset(presetName)
    ...
    if presetName=="OFF" then
        ...
        if fastFlagsApplied then task.wait(0.5); showNotification("Thông Báo Hx", "Một số cài đặt (như FastFlags) vẫn còn hiệu lực cho phiên này và không thể tự tắt bởi script.") end
...
local function createOrGetUi()
	...
	local screenGui,tempFrame; local success,result=pcall(function()
		...
		tempFrame=Instance.new("Frame",screenGui); tempFrame.Name="LagReducerFrame"; tempFrame.Size=UDim2.new(0,UI_FRAME_WIDTH,0,UI_FRAME_HEIGHT); tempFrame.AnchorPoint=Vector2.new(0.5,0); tempFrame.Position=UDim2.new(0.5,0,0,20); tempFrame.BackgroundTransparency=0.8; tempFrame.BackgroundColor3=Color3.fromRGB(30,30,30); local fc=Instance.new("UICorner",tempFrame); fc.CornerRadius=UDim.new(0,8)
