...
print("Hx Lag Reducer: Script đang tải...")

local UI_ENABLED = true
local NOTIFICATIONS_ENABLED = true
local ICON_IMAGE_ID = "rbxassetid://117118515787811"
...
local THOI_LUONG_THONG_BAO_GIAY = 3
local THOI_GIAN_HOAT_ANH_GIAY = 0.3
local KICH_THUOC_GUI_THONG_BAO = UDim2.new(0, 250, 0, 60)
local VI_TRI_KHUNG_CHUA = UDim2.new(1, -18, 1, -48)
local KICH_THUOC_KHUNG_CHUA = UDim2.new(0, 270, 0, 200)
local NOTIFICATION_TITLE_FONT = Enum.Font.SourceSansBold
local NOTIFICATION_MESSAGE_FONT = Enum.Font.SourceSans
...
local function xoaTweenKhoiTheoDoi(tweenInstance)
	if cacTweenDangHoatDong[tweenInstance] then cacTweenDangHoatDong[tweenInstance] = nil end
end

local function taoMauThongBao()
	if mauThongBao and mauThongBao.Parent == nil then return mauThongBao end
	local frame = Instance.new("Frame"); frame.Name = "NotificationFrameTemplate"; frame.BackgroundColor3 = Color3.fromRGB(30, 30, 30); frame.BackgroundTransparency = 1; frame.BorderSizePixel = 0; frame.Size = KICH_THUOC_GUI_THONG_BAO; frame.ClipsDescendants = true; mauThongBao = frame
	...
	local icon = Instance.new("ImageLabel"); icon.Name = "Icon"; icon.Image = ICON_IMAGE_ID; icon.BackgroundTransparency = 1; icon.ImageTransparency = 1; icon.Size = UDim2.new(0, 40, 0, 40); icon.LayoutOrder = 1; icon.Parent = frame
	local textFrame = Instance.new("Frame"); textFrame.Name = "TextFrame"; textFrame.BackgroundTransparency = 1; textFrame.Size = UDim2.new(1, -50, 1, 0); textFrame.LayoutOrder = 2; textFrame.Parent = frame
	...
	local title = Instance.new("TextLabel"); title.Name = "Title"; title.Text = "Tiêu đề"; title.Font = NOTIFICATION_TITLE_FONT; title.TextSize = 15; title.TextColor3 = Color3.fromRGB(255, 255, 255); title.BackgroundTransparency = 1; title.TextTransparency = 1; title.TextXAlignment = Enum.TextXAlignment.Left; title.Size = UDim2.new(1, 0, 0, 18); title.LayoutOrder = 1; title.Parent = textFrame
	local message = Instance.new("TextLabel"); message.Name = "Message"; message.Text = "Nội dung tin nhắn."; message.Font = NOTIFICATION_MESSAGE_FONT; message.TextSize = 13; message.TextColor3 = Color3.fromRGB(200, 200, 200); message.BackgroundTransparency = 1; message.TextTransparency = 1; message.TextXAlignment = Enum.TextXAlignment.Left; message.TextWrapped = true; message.Size = UDim2.new(1, 0, 1, 0); message.LayoutOrder = 2; message.Parent = textFrame
	return mauThongBao
end

local function thietLapKhungChuaThongBao()
	if khungChuaThongBao and khungChuaThongBao.Parent and giaoDienManHinhThongBao and giaoDienManHinhThongBao.Parent then return khungChuaThongBao end
	if not player or not player:IsDescendantOf(Players) then warn("Hx Lag Reducer: Người chơi cục bộ không hợp lệ."); return nil end
	playerGui = player:FindFirstChild("PlayerGui"); if not playerGui then playerGui = player:WaitForChild("PlayerGui", 10) end; if not playerGui then warn("Hx Lag Reducer: Không tìm thấy PlayerGui."); return nil end
	local oldGui = playerGui:FindFirstChild("HxNotificationGui"); if oldGui then oldGui:Destroy() end
	giaoDienManHinhThongBao = Instance.new("ScreenGui"); giaoDienManHinhThongBao.Name = "HxNotificationGui"; giaoDienManHinhThongBao.ResetOnSpawn = false; giaoDienManHinhThongBao.ZIndexBehavior = Enum.ZIndexBehavior.Sibling; giaoDienManHinhThongBao.DisplayOrder = 999; giaoDienManHinhThongBao.Parent = playerGui
	local container = Instance.new("Frame"); container.Name = "NotificationContainerFrame"; container.AnchorPoint = Vector2.new(1, 1); container.Position = VI_TRI_KHUNG_CHUA; container.Size = KICH_THUOC_KHUNG_CHUA; container.BackgroundTransparency = 1; container.Parent = giaoDienManHinhThongBao
	...
	khungChuaThongBao = container; print("Hx Lag Reducer: Khung chứa thông báo đã sẵn sàng."); return khungChuaThongBao
end

local function showNotification(tieuDe, noiDung)
	if not NOTIFICATIONS_ENABLED or not dangChay then return end
	if not khungChuaThongBao or not khungChuaThongBao.Parent then if not thietLapKhungChuaThongBao() then return end end
	if not mauThongBao then if not taoMauThongBao() then return end end
	local khungMoi = mauThongBao:Clone(); if not khungMoi then warn("Hx Lag Reducer: Không thể clone mẫu thông báo."); return end
	local hinhIcon = khungMoi:FindFirstChild("Icon"); local khungChu = khungMoi:FindFirstChild("TextFrame"); local nhanTieuDe = khungChu and khungChu:FindFirstChild("Title"); local nhanNoiDung = khungChu and khungChu:FindFirstChild("Message")
	if not (hinhIcon and nhanTieuDe and nhanNoiDung) then warn("Hx Lag Reducer: Khung thông báo clone bị lỗi cấu trúc."); khungMoi:Destroy(); return end
	nhanTieuDe.Text = tieuDe or "Thông báo"; nhanNoiDung.Text = noiDung or ""; khungMoi.Name = "Notification_" .. (tieuDe or "Default"):gsub("%s+", "_") .. "_" .. math.random(1, 1000); khungMoi.Parent = khungChuaThongBao
	local thongTinTweenXuatHien = TweenInfo.new(THOI_GIAN_HOAT_ANH_GIAY, Enum.EasingStyle.Sine, Enum.EasingDirection.Out); local tweenMoDanKhung = TweenService:Create(khungMoi, thongTinTweenXuatHien, { BackgroundTransparency = 0.2 }); local tweenMoDanIcon = TweenService:Create(hinhIcon, thongTinTweenXuatHien, { ImageTransparency = 0 }); local tweenMoDanTieuDe = TweenService:Create(nhanTieuDe, thongTinTweenXuatHien, { TextTransparency = 0 }); local tweenMoDanNoiDung = TweenService:Create(nhanNoiDung, thongTinTweenXuatHien, { TextTransparency = 0 })
	cacTweenDangHoatDong[tweenMoDanKhung] = true; cacTweenDangHoatDong[tweenMoDanIcon] = true; cacTweenDangHoatDong[tweenMoDanTieuDe] = true; cacTweenDangHoatDong[tweenMoDanNoiDung] = true
	tweenMoDanKhung.Completed:Connect(function() xoaTweenKhoiTheoDoi(tweenMoDanKhung) end); tweenMoDanIcon.Completed:Connect(function() xoaTweenKhoiTheoDoi(tweenMoDanIcon) end); tweenMoDanTieuDe.Completed:Connect(function() xoaTweenKhoiTheoDoi(tweenMoDanTieuDe) end); tweenMoDanNoiDung.Completed:Connect(function() xoaTweenKhoiTheoDoi(tweenMoDanNoiDung) end)
	tweenMoDanKhung:Play(); tweenMoDanIcon:Play(); tweenMoDanTieuDe:Play(); tweenMoDanNoiDung:Play()
	task.delay(THOI_LUONG_THONG_BAO_GIAY, function() if not dangChay or not khungMoi or not khungMoi.Parent then xoaTweenKhoiTheoDoi(tweenMoDanKhung); xoaTweenKhoiTheoDoi(tweenMoDanIcon); xoaTweenKhoiTheoDoi(tweenMoDanTieuDe); xoaTweenKhoiTheoDoi(tweenMoDanNoiDung); return end; local thongTinTweenBienMat = TweenInfo.new(THOI_GIAN_HOAT_ANH_GIAY, Enum.EasingStyle.Sine, Enum.EasingDirection.In); local tweenMoDanKhung_Mat = TweenService:Create(khungMoi, thongTinTweenBienMat, { BackgroundTransparency = 1 }); local tweenMoDanIcon_Mat = TweenService:Create(hinhIcon, thongTinTweenBienMat, { ImageTransparency = 1 }); local tweenMoDanTieuDe_Mat = TweenService:Create(nhanTieuDe, thongTinTweenBienMat, { TextTransparency = 1 }); local tweenMoDanNoiDung_Mat = TweenService:Create(nhanNoiDung, thongTinTweenBienMat, { TextTransparency = 1 }); cacTweenDangHoatDong[tweenMoDanKhung_Mat] = true; cacTweenDangHoatDong[tweenMoDanIcon_Mat] = true; cacTweenDangHoatDong[tweenMoDanTieuDe_Mat] = true; cacTweenDangHoatDong[tweenMoDanNoiDung_Mat] = true; tweenMoDanKhung_Mat.Completed:Connect(function() xoaTweenKhoiTheoDoi(tweenMoDanKhung_Mat); xoaTweenKhoiTheoDoi(tweenMoDanIcon_Mat); xoaTweenKhoiTheoDoi(tweenMoDanTieuDe_Mat); xoaTweenKhoiTheoDoi(tweenMoDanNoiDung_Mat); if khungMoi and khungMoi.Parent then khungMoi:Destroy() end end); tweenMoDanKhung_Mat:Play(); tweenMoDanIcon_Mat:Play(); tweenMoDanTieuDe_Mat:Play(); tweenMoDanNoiDung_Mat:Play() end)
end
...
local function storeOriginalSettings()
	...
	local success, err = pcall(function()
		...
		pcall(function() if UserSettings and UserSettings.GameSettings then originalQualityLevel = UserSettings.GameSettings.SavedQualityLevel elseif typeof(settings) == "function" then warn("Hx Lag Reducer: Không thể lưu cài đặt chất lượng gốc từ API settings() cũ.") else warn("Hx Lag Reducer: Không thể truy cập UserSettings để lưu chất lượng gốc.") end end)
	end)
	if not success then warn("Hx Lag Reducer: Lỗi khi lưu cài đặt gốc!", err) end
end
...
local function forceLowQuality(isReverting)
	...
	if not isReverting and not CONFIG.FORCE_LOWEST_QUALITY then return end; if not targetSavedQuality then warn("Hx Lag Reducer: Không thể xác định giá trị SavedQualitySetting đích."); return end
	local success, err = pcall(function() if UserSettings and UserSettings.GameSettings then UserSettings.GameSettings.SavedQualityLevel = targetSavedQuality elseif typeof(settings) == "function" then warn("Hx Lag Reducer: API settings() cũ không được hỗ trợ cập nhật SavedQualitySetting.") else error("Cannot access UserSettings or settings() to change quality level.") end end)
	if not success then warn("Hx Lag Reducer: Lỗi đặt mức chất lượng đồ họa (SavedQualityLevel)! ", err) end
end
...
local function applyAllOptimizations(is_rerun)
	...
	for i = #descendants, 1, -1 do local obj = descendants[i]; if obj and obj.Parent then optimizeObject(obj) end; if tick() - startOptimizeTime > 0.05 then task.wait(); startOptimizeTime = tick() end end
	if CONFIG.DELETE_UI then for _, p in pairs(Players:GetPlayers()) do local pGui = p:FindFirstChild("PlayerGui"); if pGui then local guis = pGui:GetChildren(); for i = #guis, 1, -1 do if guis[i] and guis[i]:IsA("ScreenGui") then optimizeObject(guis[i]) end end end end end
	forceLowQuality(false); if not listenerConnected and CONFIG.OPTIMIZE_ON_ADD and currentPresetName ~= "OFF" then listenerConnected = true; if descendantAddedConn then descendantAddedConn:Disconnect() end; descendantAddedConn = game.DescendantAdded:Connect(function(descendant) if optimizeAddDebounce then return end; optimizeAddDebounce = true; optimizeObject(descendant); task.delay(CONFIG.OPTIMIZE_ADD_COOLDOWN, function() optimizeAddDebounce = false end) end) elseif listenerConnected and (not CONFIG.OPTIMIZE_ON_ADD or currentPresetName == "OFF") then if descendantAddedConn then descendantAddedConn:Disconnect() end; descendantAddedConn = nil; listenerConnected = false end
end

local function applyPreset(presetName)
	local notificationMessage = "Đang chuyển sang chế độ: " .. presetName; if presetName == "OFF" then notificationMessage = "Đang tắt tối ưu hóa..." end; showNotification("Hx Lag Reducer", notificationMessage)
	if descendantAddedConn then descendantAddedConn:Disconnect(); descendantAddedConn = nil; listenerConnected = false end; optimizedObjects = setmetatable({}, { __mode = "k" })
	...
	currentPresetName = presetName; if presetName == "OFF" then CONFIG = defaultConfig; CONFIG.OPTIMIZE_ON_ADD = false; optimizeLighting(true); optimizeTerrain(true); forceLowQuality(true) else local presetConfigData = PRESETS[presetName]; if presetConfigData then CONFIG = table.clone(defaultConfig); CONFIG.OPTIMIZE_ON_ADD = true; for key, value in pairs(presetConfigData) do if CONFIG[key] ~= nil then CONFIG[key] = value else warn("Hx Lag Reducer: Không rõ key config trong preset", presetName, ":", key) end end; applyAllOptimizations(true) else warn("Hx Lag Reducer: Preset không tìm thấy:", presetName, ". Đang quay về OFF."); applyPreset("OFF"); return end end
	if lagReducerFrame and lagReducerFrame.Parent and lagReducerFrame:FindFirstChild("StatusLabel") then lagReducerFrame.StatusLabel.Text = "Hx: " .. currentPresetName end
end
...
local function createOrGetUI()
	...
	if not playerGui or not playerGui.Parent then playerGui = player:WaitForChild("PlayerGui", 15); if not playerGui then warn("Hx Lag Reducer: PlayerGui không tìm thấy trong createOrGetUI."); return nil end end

	local existingScreenGui = playerGui:FindFirstChild("LagReducerScreenGui")
	if existingScreenGui and existingScreenGui:FindFirstChild("LagReducerFrame") then
		lagReducerFrame = existingScreenGui:FindFirstChild("LagReducerFrame")
		if lagReducerFrame:FindFirstChild("StatusLabel") then lagReducerFrame.StatusLabel.Text = "Hx: " .. currentPresetName end
		return lagReducerFrame
	elseif existingScreenGui then existingScreenGui:Destroy() end
	...
	local success, result = pcall(function()
		...
		tempLagReducerFrame.Draggable = false; tempLagReducerFrame.Active = false
		...
		local iconButton = Instance.new("ImageButton"); iconButton.Name = "IconButton"; iconButton.Size = NEW_ICON_SIZE; iconButton.Position = UDim2.new(0.5, 0, 0, 5); iconButton.AnchorPoint = Vector2.new(0.5, 0); iconButton.BackgroundColor3 = Color3.fromRGB(50, 50, 50); iconButton.BackgroundTransparency = 0.3; iconButton.Image = ICON_IMAGE_ID; iconButton.Visible = true; iconButton.ZIndex = 2; iconButton.BorderSizePixel = 0; iconButton.Active = true
	...
	if not success then
		warn("Hx Lag Reducer: LỖI TRONG PCALL KHI TẠO UI NÚT BẤM! Error:", result)
		if screenGui and screenGui.Parent == nil then screenGui:Destroy() end
		lagReducerFrame = nil
		return nil
	else
		print("Hx Lag Reducer: UI Nút bấm đã sẵn sàng.")
		lagReducerFrame = result
		return result
	end
end

if NOTIFICATIONS_ENABLED then taoMauThongBao() end

if RunService:IsClient() then
	playerGui = player:WaitForChild("PlayerGui", 60); if not playerGui then warn("Hx Lag Reducer: PlayerGui không tìm thấy! Script không thể khởi tạo."); return end
	if NOTIFICATIONS_ENABLED then thietLapKhungChuaThongBao() end; showNotification("Hx Lag Reducer", "Khởi động!")
	if not player.Character or not player.Character.Parent then player.CharacterAdded:Wait(); task.wait(0.5) end
	if SELECTED_PRESET ~= "Custom" and PRESETS[SELECTED_PRESET] then currentPresetName = SELECTED_PRESET else currentPresetName = "OFF" end
	storeOriginalSettings();
	createOrGetUI();
	applyPreset(currentPresetName);
	print("Hx Lag Reducer: Khởi tạo hoàn tất.")
else warn("Hx Lag Reducer: Script phải là LocalScript trong StarterPlayerScripts!") end
